#!/usr/bin/env bash
#
# Finance Copilot Equity Research — Google Cloud Run One-Click Deployment
#
# Prerequisites:
#   - gcloud CLI installed and authenticated
#   - config.ini with real API keys at finance_copilot_equity/core/config/config.ini
#
# Usage:
...
BUCKET_NAME="finance_copilot-reports-${PROJECT_ID}"
SQL_INSTANCE_NAME="finance_copilot-db"
DB_NAME="finance_copilot"
DB_USER="finance_copilot"
DB_PASS="$(openssl rand -base64 24)"         # random password, stored in secret
SECRET_NAME="finance_copilot-config-ini"
CONFIG_INI_PATH="finance_copilot_equity/core/config/config.ini"

echo "=========================================="
echo " Finance Copilot Cloud Run Deployment"
echo "=========================================="
...
gcloud run deploy "${SERVICE_NAME}" \
    ...
    --update-secrets="/app/finance_copilot_equity/core/config/config.ini=${SECRET_NAME}:latest" \
    --execution-environment=gen2 \
    --add-volume=name=reports,type=cloud-storage,bucket="${BUCKET_NAME}" \
    --add-volume-mount=volume=reports,mount-path=/app/finance_copilot_equity/core/output \
    --quiet
