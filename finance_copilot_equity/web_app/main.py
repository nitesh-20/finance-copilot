...
app = FastAPI(title="Finance Copilot Equity Research", version="1.0.0")
...
async def run_analysis(req: AnalysisRequest, request: Request, background_tasks: BackgroundTasks):
    ...
    # Sanitize inputs to prevent trailing whitespaces or spaces breaking symbol lookups
    req.ticker = req.ticker.strip() if req.ticker else ""
    req.company_name = req.company_name.strip() if req.company_name else ""
    if req.peers:
        req.peers = [p.strip() for p in req.peers if p.strip()]
...
async def list_all_logs(request: Request):
    ...
    # 只有管理员可以查看所有日志
    # Admin emails can be configured via FINANCE_COPILOT_ADMIN_EMAILS env var (comma-separated)
    admin_emails = os.getenv("FINANCE_COPILOT_ADMIN_EMAILS", "admin@finance_copilot.com").split(",")
