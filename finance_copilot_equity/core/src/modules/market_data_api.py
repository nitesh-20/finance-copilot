...
def get_fmp_ratios_and_key_metrics(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    try:
        ...
        if ratios_data:
            ...
            if "fiscalYear" in ratios_df.columns:
                ratios_df["calendarYear"] = ratios_df["fiscalYear"]
            if "debtToEquityRatio" in ratios_df.columns:
                ratios_df["debtEquityRatio"] = ratios_df["debtToEquityRatio"]
            if "debtToAssetsRatio" in ratios_df.columns:
                ratios_df["debtRatio"] = ratios_df["debtToAssetsRatio"]
            if "interestCoverageRatio" in ratios_df.columns:
                ratios_df["interestCoverage"] = ratios_df["interestCoverageRatio"]
            if "operatingCashFlowCoverageRatio" in ratios_df.columns:
                ratios_df["cashFlowToDebtRatio"] = ratios_df["operatingCashFlowCoverageRatio"]
            # Fill in any missing default columns to prevent validation failure
            for expected_col in ["debtEquityRatio", "debtRatio", "interestCoverage", "netProfitMargin", "currentRatio", "cashFlowToDebtRatio"]:
                if expected_col not in ratios_df.columns:
                    ratios_df[expected_col] = 0.0
        ...
        if key_metrics_data:
            ...
            if "fiscalYear" in key_metrics_df.columns:
                key_metrics_df["calendarYear"] = key_metrics_df["fiscalYear"]
...
def get_yfinance_financial_data(ticker: str) -> dict:
    """Fallback function to fetch statements from yfinance if FMP is not available."""
    ...
    try:
        ...
        try:
            ...
            if is_df is not None and not is_df.empty:
                is_df_trans = is_df.transpose()
                is_df_trans.reset_index(inplace=True)
                rename_map = {}
                for col in is_df_trans.columns:
                    col_str = str(col).lower().replace(' ', '').replace('_', '')
                    if 'revenue' in col_str or 'sales' in col_str:
                        if 'costof' not in col_str and 'operating' not in col_str:
                            rename_map[col] = 'revenue'
                    elif 'netincome' in col_str:
                        rename_map[col] = 'netIncome'
                    elif 'ebitda' in col_str:
                        rename_map[col] = 'ebitda'
                    elif 'operatingincome' in col_str or 'ebit' in col_str:
                        if 'ebitda' not in col_str:
                            rename_map[col] = 'operatingIncome'
                    elif 'grossprofit' in col_str:
                        rename_map[col] = 'grossProfit'
                    elif 'index' in str(col) or 'date' in str(col).lower():
                        rename_map[col] = 'date'
                
                is_df_trans.rename(columns=rename_map, inplace=True)
                is_df_trans['date'] = pd.to_datetime(is_df_trans['date'])
                is_df_trans['year'] = is_df_trans['date'].dt.year
                is_df_trans['calendarYear'] = is_df_trans['year'].astype(str)
                
                for col in ['revenue', 'netIncome', 'ebitda']:
                    if col not in is_df_trans.columns:
                        is_df_trans[col] = 0.0
                data['income_statement'] = is_df_trans
        except Exception as e:
            print(f"yfinance income statement fetch failed for {ticker}: {e}")

        # 2. Balance sheet
        try:
            bs_df = t.balance_sheet
            if bs_df is not None and not bs_df.empty:
                bs_df_trans = bs_df.transpose()
                bs_df_trans.reset_index(inplace=True)
                rename_map = {}
                for col in bs_df_trans.columns:
                    col_str = str(col).lower().replace(' ', '').replace('_', '')
                    if 'assets' in col_str:
                        rename_map[col] = 'totalAssets'
                    elif 'liabilities' in col_str:
                        rename_map[col] = 'totalLiabilities'
                    elif 'equity' in col_str:
                        rename_map[col] = 'totalStockholdersEquity'
                    elif 'cash' in col_str:
                        rename_map[col] = 'cashAndCashEquivalents'
                    elif 'index' in str(col) or 'date' in str(col).lower():
                        rename_map[col] = 'date'
                bs_df_trans.rename(columns=rename_map, inplace=True)
                bs_df_trans['date'] = pd.to_datetime(bs_df_trans['date'])
                bs_df_trans['year'] = bs_df_trans['date'].dt.year
                bs_df_trans['calendarYear'] = bs_df_trans['year'].astype(str)
                data['balance_sheet'] = bs_df_trans
        except Exception as e:
            print(f"yfinance balance sheet fetch failed for {ticker}: {e}")

        # 3. Cash flow
        try:
            cf_df = t.cashflow
            if cf_df is not None and not cf_df.empty:
                cf_df_trans = cf_df.transpose()
                cf_df_trans.reset_index(inplace=True)
                rename_map = {}
                for col in cf_df_trans.columns:
                    col_str = str(col).lower().replace(' ', '').replace('_', '')
                    if 'operating' in col_str and 'cash' in col_str:
                        rename_map[col] = 'operatingCashFlow'
                    elif 'capital' in col_str and 'expend' in col_str:
                        rename_map[col] = 'capitalExpenditure'
                    elif 'free' in col_str and 'cash' in col_str:
                        rename_map[col] = 'freeCashFlow'
                    elif 'index' in str(col) or 'date' in str(col).lower():
                        rename_map[col] = 'date'
                cf_df_trans.rename(columns=rename_map, inplace=True)
                cf_df_trans['date'] = pd.to_datetime(cf_df_trans['date'])
                cf_df_trans['year'] = cf_df_trans['date'].dt.year
                cf_df_trans['calendarYear'] = cf_df_trans['year'].astype(str)
                data['cash_flow'] = cf_df_trans
        except Exception as e:
            print(f"yfinance cash flow fetch failed for {ticker}: {e}")
...
def get_comprehensive_financial_data(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> dict:
    ...
    # Check if FMP failed to fetch income statement, and fallback to yfinance
    if financial_data['income_statement'] is None or financial_data['income_statement'].empty:
        yf_data = get_yfinance_financial_data(ticker)
        if yf_data['income_statement'] is not None and not yf_data['income_statement'].empty:
            print(f"✅ Successfully loaded financial statements via yfinance fallback for {ticker}")
            financial_data['income_statement'] = yf_data['income_statement']
            financial_data['balance_sheet'] = yf_data['balance_sheet']
            financial_data['cash_flow'] = yf_data['cash_flow']
...
def get_fmp_target_price(ticker: str, api_key: str) -> float | None:
    ...
    try:
        ...
        if data and isinstance(data, list) and len(data) > 0:
            ...
            print(f"No target price consensus returned from FMP for {ticker}. Response: {data}")
            return None
    except requests.exceptions.RequestException as e:
...
def get_comprehensive_company_metrics(ticker: str, api_key: str) -> dict:
    ...
    if profile:
        metrics['market_cap'] = profile.get('marketCap', 0) / 1e9 if profile.get('marketCap') else None  # Convert to billions
        metrics['volume'] = profile.get('averageVolume', 0) / 1e6 if profile.get('averageVolume') else None  # Convert to millions
        metrics['beta'] = profile.get('beta')
        metrics['sector'] = profile.get('sector', 'N/A')
        metrics['industry'] = profile.get('industry', 'N/A')
        metrics['exchange'] = profile.get('exchange', 'N/A')
    ...
    try:
        ...
        if ratios_df is not None and not ratios_df.empty:
            ...
            metrics['net_debt_to_equity'] = latest_ratios.get('debtToEquityRatio') or latest_ratios.get('debtEquityRatio')
            if latest_ratios.get('priceToEarningsRatio') is not None:
                metrics['fwd_pe'] = float(latest_ratios['priceToEarningsRatio'])
            elif latest_ratios.get('priceEarningsRatio') is not None:
                metrics['fwd_pe'] = float(latest_ratios['priceEarningsRatio'])
        ...
        if key_metrics_df is not None and not key_metrics_df.empty:
            ...
            if latest_key_metrics.get('returnOnEquity') is not None:
                metrics['roe'] = float(latest_key_metrics['returnOnEquity']) * 100  # Convert to percentage
            if latest_key_metrics.get('peRatio') is not None:
                metrics['fwd_pe'] = float(latest_key_metrics['peRatio'])
            if latest_key_metrics.get('pbRatio') is not None:
                metrics['pb_ratio'] = float(latest_key_metrics['pbRatio'])
    except Exception as e:
        print(f"Warning: Could not fetch financial ratios: {e}")
    
    # 6. Get dividend yield from profile or financial data
    if profile and profile.get('lastDividend'):
        if current_price and profile['lastDividend'] > 0:
            # Calculate dividend yield: (annual dividend / current price) * 100
            annual_dividend = profile['lastDividend']
            metrics['dividend_yield'] = (annual_dividend / current_price) * 100
    
    # 7. Calculate/fetch shares outstanding and free float
    if metrics['market_cap'] and metrics['share_price'] and metrics['share_price'] > 0:
        metrics['shares_outstanding'] = (metrics['market_cap'] * 1e9) / metrics['share_price']
    
    try:
        if profile and (profile.get('sharesOutstanding') or profile.get('marketCap')):
            metrics['free_float'] = 99.0  # Default assumption for large public companies
    except Exception as e:
        print(f"Warning: Error calculating free float: {e}")
