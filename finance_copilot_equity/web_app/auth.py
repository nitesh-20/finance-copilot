...
def init_default_admin():
    ...
    Admin credentials can be configured via environment variables:
        FINANCE_COPILOT_ADMIN_EMAIL (default: admin@finance_copilot.com)
        FINANCE_COPILOT_ADMIN_PASSWORD (default: randomly generated)
    ...
    try:
        ...
        if stats["total_users"] == 0:
            admin_email = os.getenv("FINANCE_COPILOT_ADMIN_EMAIL", "admin@finance_copilot.com")
            admin_password = os.getenv("FINANCE_COPILOT_ADMIN_PASSWORD", secrets.token_urlsafe(12))
            crud.create_user(
                db=db,
                email=admin_email,
            ...
            if not os.getenv("FINANCE_COPILOT_ADMIN_PASSWORD"):
