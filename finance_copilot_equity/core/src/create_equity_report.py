...
def validate_and_fix_text_content(text_content: str, text_type: str, company_name: str, company_ticker: str) -> str:
    ...
    if not text_content or text_content.strip() == "":
        ...
        if text_type == "news_summary":
            return "No recent news or sentiment updates available for this stock."
        return ""
...
def regenerate_text_if_needed(text_content: str, text_type: str, company_name: str, company_ticker: str, 
    ...
    is_empty = not text_content or text_content.strip() == ""
    valid_text_types = [
    ...
    # Generate/regenerate using AI if key is present, or if it is empty/insufficient.
    if text_type in valid_text_types and (api_key or is_empty):
        try:
            ...
            print(f"🤖 Generating/Regenerating '{text_type}'...")
            ...
            if generated_text and len(generated_text.strip()) > 50:
                print(f"✅ Successfully generated/regenerated '{text_type}'")
                return generated_text
            else:
                print(f"❌ Generated text for '{text_type}' is insufficient, using original or fallback.")
            
        except Exception as e:
            print(f"❌ Failed to generate '{text_type}': {e}")
            
    # If not generated/regenerated, validate original content
    fixed_text = validate_and_fix_text_content(text_content, text_type, company_name, company_ticker)
    
    # If original content is still empty/invalid, raise an error
    if not fixed_text or fixed_text.strip() == "":
        raise ValueError(f"Required text content for '{text_type}' is empty, and live generation failed/disabled.")
...
def main():
    ...
    parser.add_argument("--research-source", type=str, default="Finance Copilot Equity Research", help="Source of the research.")
    parser.add_argument("--data-source-text", type=str, default="Company Filings, FMP, Yahoo Finance, Finance Copilot Estimates", help="Text for data sources.")
    ...
    args.company_ticker = args.company_ticker.strip() if args.company_ticker else ""
    args.company_name = args.company_name.strip() if args.company_name else ""
